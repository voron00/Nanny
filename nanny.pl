		...
		# !time
			...
			if (&check_access('time')) {
				if (&flood_protection('time', 30, $slot)) { }
					...
					&rcon_command("say Current time: ^2$timestring");
				}
			}
		}
...
sub banned_player_kick {
	...
	if (!$ban_message_spam) {
		...
		sleep 1;
		&rcon_command("say $ban_name^7: Was banned ^2$bantime ^7(BAN ID#: ^1$ban_id^7)");
		sleep 1;
...
sub clear_names {
	...
	if ($#matches == 0) {
		...
		if ($result) {
			...
			$ip_to_name_sth->execute($ip_by_slot{$matches[0]})
			  or &die_nice("Unable to execute query: $ip_to_name_dbh->errstr\n");
			&rcon_command("say Removed ^1$result ^7names for: $name_by_slot{$matches[0]}");
			&log_to_file('logs/admin.log', "!CLEARNAMES: $name_by_slot{$matches[0]} names were deleted by $name - GUID $guid (Search: $search_string)");
		}
		else { &rcon_command("say No names found for: $name_by_slot{$matches[0]}"); }
	}
	elsif ($#matches == -1) {
		&rcon_command("say No matches for: $search_string");
		return 1;
			...
			if ($name_by_slot{$slot} ne $name) {
					...
					and ($config->{'ban_name_thieves'}))
				{
					...
					if (($old_name_stolen) and ($new_name_stolen) and (!$ban_message_spam)) {
						&rcon_command("say ^1NAME STEALING DETECTED^7: Slot #^3$slot ^7was permanently banned for name stealing!");
